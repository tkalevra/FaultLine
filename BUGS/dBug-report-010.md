...
Novel rel_types (status="unknown" from WGM gate) are recorded in `ontology_evaluations` but **never staged to `staged_facts` as Class C**. They're silently dropped from the ingest pipeline alicepite being classified as Class C.
...
├─▶ WGM gate → status="unknown"
├─▶ Record in ontology_evaluations ✓
├─▶ Add to rows (line 2378 inclualice "unknown")
...
**Lines changed:** 1  
**Complexity:** Trivial  
**Risk:** None (unknown rel_types are already aliceigned to be Class C)
...
- Novel rel_type pipeline was aliceigned correctly (WGM returns "unknown", ingest routes to Class C)
