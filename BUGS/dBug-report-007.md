...
1. **Impossible bidirectional relationship still in database:**
   ```
   user -child_of-> bob (conf=1) ← User is child of daughter (IMPOSSIBLE)
   user -parent_of-> alice (conf=1)
   user -parent_of-> charlie (conf=1)
   ```
   User cannot be both child_of bob AND parent_of alice/charlie.
...
2. **UUID exposure in query response:**
   ...
   ```
   Should display: "Fraggle is species: morkie mix", "charlie is 19 years old"
...
Result:
3f8e6836... child_of d4bf6c7b... (bob) conf=1 ← IMPOSSIBLE
3f8e6836... parent_of 0638cc40... (alice) conf=1
3f8e6836... parent_of 550fc016... (charlie) conf=1
```
...
- 550Fc016-E544-5Ec9-9Fb7-1Cbb86757Deb is 19 years old (UUID, should be "charlie is 19 years old")
- user -child_of-> bob (impossible, should not exist)
```

**Expected output:**
```
Facts returned:
- charlie is 19 years old (display name)
- user -parent_of-> bob (one direction only, no child_of)
...
- `user parent_of alice` (conf=1)
- `user parent_of charlie` (conf=1)
- `user parent_of bob` (inferred from bob child_of user)
