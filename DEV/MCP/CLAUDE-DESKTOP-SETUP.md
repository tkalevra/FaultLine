...
2. **`FAULTLINE_USER_ID`** — your user UUID as registered in FaultLine's PostgreSQL. For the docker-host pre-prod instance this is `10d7d879-63cd-4f31-92ce-f2c9edb760ab`. To look it up on any instance:
   ```bash
...
```json
{
  "mcpServers": {
    "faultline": {
      "command": "python",
      "args": ["/home/chris/Documents/013-GIT/FaultLine-dev/tools/mcp_server.py"],
      "env": {
        "FAULTLINE_USER_ID": "10d7d879-63cd-4f31-92ce-f2c9edb760ab",
        "FAULTLINE_API_URL": "http://YOUR-TRUENAS-IP:8000"
      }
    }
  }
}
...
OpenWebUI connects to FaultLine's tools via its **OpenAPI** connection type (not native MCP). The MCP HTTP server exposes dedicated REST endpoints that OpenWebUI discovers via `/openapi.json`.

In OpenWebUI: **Settings → Connections → Tools → Add Connection**

| Field | Value |
|-------|-------|
| Type | `OpenAPI` |
| Name | `Faultline-mcp` |
| URL | `http://faultline-mcp:8002` (Docker-internal) |
| Auth | `Bearer` + `<your MCP_API_KEY value>` |
| OpenAPI Spec | `URL` → `openapi.json` (default — leave as-is) |

OpenWebUI fetches `http://faultline-mcp:8002/openapi.json`, discovers `recall_memory`, `remember_facts`, and `retract_fact` as callable REST endpoints, and surfaces them as native tools in every conversation. The Filter (`faultline_function.py`) continues to handle automatic memory injection in parallel.

> **Note:** The `/mcp` JSON-RPC endpoint is still available for Claude Desktop (Model 2b below) and any MCP-native client. OpenWebUI uses the REST endpoints instead.
