...
ssh docker-host -x "sudo docker logs faultline --tail 100"
ssh docker-host -x "sudo docker logs open-webui --tail 100"
ssh docker-host -x "sudo docker logs faultline-postgres --tail 50"

# Query database (read-only)
ssh docker-host -x "sudo docker exec faultline-postgres psql -U faultline -d faultline \
  -c 'SELECT COUNT(*) FROM facts WHERE rel_type = '\''has_pet'\'';'"

# Check health
ssh docker-host -x "sudo docker exec open-webui curl -s http://faultline:8001/health"
