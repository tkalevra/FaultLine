...
def test_validate_novel_type():
    """Novel rel_type not in ontology → returns 'novel_unapproved' for Class C staging (dprompt-130)."""
    mock_conn = MagicMock()
    gate = WGMValidationGate(mock_conn)

    result = gate.validate_edge(1, 2, "unknown_relationship")

    # Novel types trigger synchronous LLM approval (dprompt-130).
    # If approval fails (confidence < 0.7), returns "novel_unapproved" for Class C staging.
    # Note: Test uses mocks, so _try_approve_novel_type will fail without proper mock setup.
    assert result["status"] in ("novel_unapproved", "valid")
