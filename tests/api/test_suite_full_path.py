...
Groups:
  ...
  B (6-11): Name collision + resolution — simple, LLM, ${CHILD3} repro, triple, scalar, re-ingest
...
class TestGroupABaseIntegration:
    ...
    def test_1_family_prose_full_path(self, app):
        ...
        # Collision check: user "${CHILD3}" vs child "${CHILD3}"
        conflicts = _db_count(self.uid, "entity_name_conflicts",
                             "AND disputed_name = '${CHILD3}'")
    ...
    def test_3_fact_correction_full_path(self, app):
        ...
        _ingest(client, "${CHILD3} is 10", self.uid)
        _ingest(client, "${CHILD3} is 11", self.uid)
    ...
    def test_4_alias_resolution_full_path(self, app):
        ...
        _ingest(client, "${CHILD3} goes by ${CHILD3}, likes pizza", self.uid, edges=edges)
    ...
    def test_5_fact_promotion_full_path(self, app):
        ...
        for i in range(3):
            _ingest(client, f"${USER} likes coffee (x{i+1})", self.uid)

        result = _query(client, "what does ${USER} like", self.uid)
        assert result.get("status") == "ok"
        facts = result.get("facts", [])
        likes = [f for f in facts if f["rel_type"] == "likes"]
        assert len(likes) >= 1
...
class TestGroupBNameCollision:
    ...
    def test_6_simple_name_collision(self, app):
        ...
        # Entity 1: user claims "${CHILD3}" as preferred
        edges1 = [{"subject": "user", "object": "${CHILD3}", "rel_type": "pref_name",
                   "subject_type": "Person", "object_type": "SCALAR"}]
        _ingest(client, "I go by ${CHILD3}", self.uid, edges=edges1)

        # Entity 2: child also claims "${CHILD3}"
        edges2 = [{"subject": "${CHILD3}", "object": "${CHILD3}", "rel_type": "pref_name",
                   "subject_type": "Person", "object_type": "SCALAR"}]
        _ingest(client, "${CHILD3} goes by ${CHILD3}", self.uid, edges=edges2)

        # Verify collision stored
        conflicts = _db_count(self.uid, "entity_name_conflicts",
                             "AND disputed_name = '${CHILD3}' AND status = 'pending'")
    ...
    def test_8_gabriella_reproduction(self, app):
        """EXACT reproduction of live bug: ${CHILD3} ingested but invisible."""
        client = TestClient(app)

        # Pre-seed: user already has pref_name="${CHILD3}"
        edges_user = [
            {"subject": "user", "object": "${CHILD3}", "rel_type": "pref_name",
             "subject_type": "Person", "object_type": "SCALAR"},
            {"subject": "user", "object": "${USER}", "rel_type": "also_known_as",
             "subject_type": "Person", "object_type": "SCALAR"},
        ]
        _ingest(client, "I go by ${CHILD3}, also ${USER}", self.uid, edges=edges_user)

        # Now ingest ${CHILD3} — same preferred name "${CHILD3}"
        text = "We have a third Daughter, ${CHILD3} who's 10 and goes by ${CHILD3}"
        ingest = _ingest(client, text, self.uid)
        assert ingest.get("status") == "valid"
        ...
        # Query — ${CHILD3} must be visible (the fix)
        result = _query(client, "tell me about my family", self.uid)
        assert result.get("status") == "ok"
        facts = result.get("facts", [])
        parent_facts = [f for f in facts if f["rel_type"] == "parent_of"]
        ...
        assert len(parent_facts) >= 1, \
            f"${CHILD3} missing — parent_of facts: {len(parent_facts)}"
    ...
    def test_10_collision_with_scalar_facts(self, app):
        ...
        edges_user = [{"subject": "user", "object": "${CHILD3}", "rel_type": "pref_name",
                       "subject_type": "Person", "object_type": "SCALAR"}]
        _ingest(client, "I go by ${CHILD3}", self.uid, edges=edges_user)

        edges_child = [
            {"subject": "${CHILD3}", "object": "${CHILD3}", "rel_type": "pref_name",
             "subject_type": "Person", "object_type": "SCALAR"},
            {"subject": "${CHILD3}", "object": "10", "rel_type": "age",
             "subject_type": "Person", "object_type": "SCALAR"},
        ]
        _ingest(client, "${CHILD3} is 10, goes by ${CHILD3}", self.uid, edges=edges_child)
...
class TestGroupDSensitivityNovel:
    ...
    def test_18_confidence_variation_full_path(self, app):
        ...
        for conf_text in ("definitely", "maybe", "possibly"):
            _ingest(client, f"${USER} {conf_text} likes tea", self.uid)
...
class TestGroupEIdempotencyEdge:
    ...
    def test_20_duplicate_ingest_10x(self, app):
        ...
        for i in range(10):
            _ingest(client, "${CHILD2} is 19", self.uid)
    ...
    def test_21_partial_reingest(self, app):
        ...
        _ingest(client, "Kids: ${CHILD2} and ${CHILD3}", self.uid, edges=edges)
        count_before = _db_count(self.uid, "facts", "AND superseded_at IS NULL")

        # Re-ingest only ${CHILD2}
        _ingest(client, "${CHILD2} is my son", self.uid)
        count_after = _db_count(self.uid, "facts", "AND superseded_at IS NULL")
        # Should not lose ${CHILD3}
