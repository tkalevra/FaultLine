...
# it began), NOT a valid-AT. The old emit passed ``temporal_status=None`` for the 'now' case, letting
# the request-level DATE-DRIVEN detector read the past move-date as 'past' and stamp the RESIDENCE
# 'past' — so "where do I live now?" lost Vancouver and "where did I used to live?" lost Toronto
# (the valid-time of the MOVE conflated with the residence-currency; bi-temporal error, Zep/Graphiti
# 2025). ON → emit the relocation's 'now' EXPLICITLY (per-edge override) so a past-dated completed
# move stays the current residence; the row-build still promotes 'now'→'future' for a FUTURE-dated
# move (see main.py). OFF → legacy None-for-now (today's byte-for-byte behavior). Grammatical-tense
# only (the relocation chain already gates the verb class), never a content-verb tense guess.
...
# gallery", "the Louvre, a museum", "Ada, a startup" — asserts the NAME is-a TYPE (Hearst 1992's
# canonical appositive hyponymy pattern; UD ``appos``). Today ``_chain_appositive`` emits only a
# generic ``(name, has_role, type)`` for this, so the named entity is NEVER ``instance_of`` its class
# and a "how many <type>" count (which grounds the type node then counts its ``instance_of`` members)
# FALSE-MISSES the venue (LME multi-session count cluster, qid 80ec1f4f 2nd operand "The Art Cube").
# When ON, that construction instead files the NAME and types it ``(name, instance_of, <head-noun>)``
# — subclassing it into L4 so it is countable — EXCEPT when the TYPE is a PERSON role (curator/agent),
# which stays ``has_role`` (a person's occupation, not a class). The person-vs-thing split is WordNet's
# ``noun.person`` lexname partition (``wordnet_ladder.is_person_noun``), deterministic + offline.
# DEFAULT ON; OFF → byte-for-byte the legacy ``has_role`` emit. Fail-safe: WordNet unavailable / an
# uncertain person-check / any error → the legacy ``has_role`` path (never a silent drop).
...
# plan is 500 Mbps") is otherwise emitted as a FLAT `(user, <noun>, <measure>)` edge — the possessed
# THING becomes a rel_type. At /ingest that novel rel mints category='pending_placement', and grounding
# it hits THE HARD LINE (entity_registry: a rel_type name cannot ALSO be a concept entity) — the value
# is LOST. Option A fixes it AT CAPTURE: the possessed noun is modeled as an ENTITY the possessor HAS,
# with the measure as a SCALAR on that entity — `(user, has, commute)` + `(commute, <scalar-rel>,
# "45 minutes")` — so no rel-typed `commute` edge is ever produced, no orphan to ground, no collision.
# THE DISCRIMINATOR is metadata-driven (the rel_type overlay), NOT a noun list: fire ONLY when the
# noun-as-rel is ABSENT from the overlay (truly novel — never minted as a rel); a SEEDED rel (age/
# height/owns/…) stays EXACTLY as today. Subject-agnostic. Shares the SPINE_PENDING_GROUNDING flag with
# the main.py harvest re-home (default OFF, bisectable). Fail-safe: flag OFF, an unreadable/empty
# overlay, a non-measure value, or ANY error → today's flat scalar emit (value never dropped).
SPINE_PENDING_GROUNDING: bool = os.environ.get(
    "SPINE_PENDING_GROUNDING", "false"
).strip().lower() in ("1", "true", "yes")

# ── TIME-OF-DAY SCHEDULE SCALAR (multi-schedule collision fix, default ON) ─────────────────────────
# A recurring CLOCK-TIME-OF-DAY bound to a day/period FRAME — "I wake up at 6:45 AM on Tuesdays and
# Thursdays", "I take my pills at 9 PM on weekends" — is captured today as TWO divorced scalars: the
# clock time on the BARE verb-lemma attribute ("wake" = "6:45 am", via the measure/verb chain) and the
# day span on the prep-qualified attribute ("wake_on" = "tuesdays and thursdays", via the schedule
# pre-pass). That is fine for ONE schedule, but a person states SEVERAL ("… at 8:30 on weekdays … at
# 6:45 on Tuesdays and Thursdays"): every clock time then lands on the SAME "wake" key and, because
# entity_attributes is UNIQUE(entity_id, attribute), each OVERWRITES the last — only one wake time
# survives and the day-scoped answer ("what time on Tuesdays and Thursdays") is lost. The frame is the
# natural DISTINGUISHER (each schedule occupies a different day/period), so when a verb carries BOTH a
# clock-time measure AND a recurring frame we ADDITIVELY emit ONE uncollided schedule-time scalar keyed
# by the frame: rel = f"{verb}_{prep}_{frame_slug}" (a minted scalar rel, same growth path the bare
# verb-lemma / "wake_on" attributes already use), value = the clock time. Distinct schedules then get
# distinct keys and ALL survive; the walk surfaces the day-matching one. This is TIMEX3's TIME (a
# time-of-day point, ISO-8601 hh:mm) vs DATE (a calendar day) distinction (Pustejovsky et al. 2003,
# TimeML/TIMEX3) — the clock time is a value on the routine, NEVER an event_date (the temporal peel
# already returns None for it) and NEVER an age (the copula-measure shape gate already rejects a ':' —
# a structured literal). The day/period recurrence is the UD nmod:tmod frame (de Marneffe et al.,
# Universal Dependencies temporal modifier). ADDITIVE + purely grammatical (NER TIME clock span + the
# existing schedule pre-pass's dateparser-firewalled recurring DATE frame) — the two legacy scalars are
# UNCHANGED, so every existing capture/test is byte-identical; this only ADDS the coexisting key.
# OFF → byte-for-byte the pre-fix two-scalar behavior. Subject-agnostic, deterministic, fail-safe.
SPINE_TIME_OF_DAY_SCHEDULE: bool = os.environ.get(
    "SPINE_TIME_OF_DAY_SCHEDULE", "true"
).strip().lower() not in ("0", "false", "no")

# MEANING-LEVEL COVERAGE GATE (HALF 1, default OFF, bisectable). Today an emitted edge CLAIMS its
# tokens for the coverage/residue check REGARDLESS of whether the edge is type-VALID — so a mis-typed
# capture (a junk twin whose object is a scalar/measure/date VALUE but whose rel is a NON-scalar
# relation, e.g. `(favorite shoes, also_known_as, "nike")`, `(commute, also_known_as, "minutes")`)
# consumes the salient token and reads as "handled": the correct meaning falls through BOTH bins (not
# correctly in A/B, not netted to C). The gate changes "covered" from "some edge claimed it" to "a
# TYPE-POSSIBLE edge claimed it": in ``_emit``, AFTER the HARD-LINE guards and BEFORE ``_claim``, an
# AGREEMENT VETO between two ORTHOGONAL signals — (1) the rel's GROWN ``tail_types`` from the per-tenant
# rel_type overlay (the native SHACL range analog, grown per-tenant, NOT a hardcoded table) and (2) the
# object token's GRAMMAR datatype computed INDEPENDENTLY of GLiNER2 (``NumType=Card``/like_num, NER
# DATE/TIME/QUANTITY/MONEY/PERCENT). VETO (return, don't emit, don't claim) ONLY on a STRUCTURAL
# CONTRADICTION: the object is a scalar/measure/date VALUE and the rel's grown tail_types has NO SCALAR
# — the span uncovers → flows to the existing residue/growth net (HALF 2). CLOSED-CLASS/structural
# only, NEVER a fuzzy re-typer; near-zero false-positive. Fail-safe: overlay miss / no tail_types /
# object already a placed structured atomic (scalar_datatype set) / ANY error → do NOT veto (today's
# behaviour, value never dropped). Flag OFF → byte-identical.
SPINE_COVERAGE_GATE: bool = os.environ.get(
...
# unchanged ``_chains`` loop + reconciles, BEFORE the residue guard — runs ``extract_propositions`` over
# the SAME parse and emits a generic edge ONLY for a content span NO chain claimed (``_covered`` twin-
# suppression, G5). Every PA edge rides the SAME ``_emit`` chokepoint the chains use (minted-rel/GLiNER2
# convergence, coverage gate, HARD-LINE guards, discourse-topic/relative-pronoun refinements, ``_claim``,
# dedup) → harvest → /ingest → node_role → WGM → growth (G4, nothing hardcoded). The chains are UNTOUCHED
# and OWN their constructions; PA fills only the gaps the chains never had a chain for (ditransitive
# recipient, dative-for, event-nominalization, tenure obl, xcomp control, nmod factoring). Copular
# clauses are SKIPPED (their semantic rel — instance_of/has_state/favorite_X/age — is a refinement the
# naming/state/measure/affect chains own; a raw ``be`` edge is junk). Read at CALL time (not module load)
# so one test process can exercise ON vs OFF. DEFAULT OFF → byte-identical to today's chain output.
# Fail-safe: flag OFF / clause_pa unavailable / ANY error → the chains' output stands unchanged.
def _pa_core_active() -> bool:
